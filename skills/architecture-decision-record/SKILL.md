...
- If not otherwise stated, write ADR in English
...
- New ADR filenames use a number greater than any existing ADR in the current folder
- ADR can be placed in a topic-related subfolder under the project directory `/docs/decisions/<topic-sub-folder>`
- The template used by this skill is at [ADR template](./references/0000-a-markdown-adr-template.md) — this is the sole authoritative template; use it for all structure and formatting decisions
  - This skill's template is based on [MADR adr-template-bare.md](https://github.com/adr/madr/blob/4.0.0/template/adr-template-bare.md) but has been customized; the upstream MADR template is for historical reference only and must NOT be used to infer structure or fill in gaps
- ADR has a title
- ADR has a status and a date when that status was decided
  - new ADR is always created with status `draft`
